    ...
    public class UserService : IUserService
    {
        ...
        public async Task<string> RegisterAsync(RegisterDto registerDto)
        {
            ...
            var existingUser = _unitOfWork
                .Users.Find(u => u.Username.ToLower() == registerDto.Username.ToLower())
                .FirstOrDefault();

            if (existingUser == null)
            {
                var rolDefault = _unitOfWork
                    .Roles.Find(u => u.Name == Authorization.rol_default.ToString())
                    .First();
                try
                {
        ...
        public async Task<DataUserDto> GetTokenAsync(LoginDto loginDto)
        {
            ...
            if (result == PasswordVerificationResult.Success)
            {
                ...
                {
                    var activeRefreshToken = user
                        .RefreshTokens.Where(a => a.IsActive == true)
                        .FirstOrDefault();
        ...
        public async Task<string> AddRoleAsync(AddRoleDto addRoleDto)
        {
            ...
            if (result == PasswordVerificationResult.Success)
            {
                var rolExists = _unitOfWork
                    .Roles.Find(u => u.Name.ToLower() == addRoleDto.Role.ToLower())
                    .FirstOrDefault();
